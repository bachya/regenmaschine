...
from .provisioning import provisioning_name_json, provisioning_wifi_json


@pytest.fixture()
def authenticated_client(
        auth_login_json, event_loop, provisioning_name_json,
        provisioning_wifi_json):
    ...
    client.add(
        ...
        aresponses.Response(
            text=json.dumps(provisioning_name_json), status=200))
    client.add(
        '{0}:{1}'.format(TEST_HOST, TEST_PORT), '/api/4/provision/wifi', 'get',
        aresponses.Response(
            text=json.dumps(provisioning_wifi_json), status=200))
